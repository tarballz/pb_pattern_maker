...
export function render3D(index, x, y, z) {
  // 2D version uses translate(0.5,-0.5); apply that to x,y and center z.
  var cx = x + 0.5;
  var cy = y - 0.5;
  var cz = z - 0.5;

  var rr = wave(t1 + (cx * waveMult[0] + cy * waveMult[1] + cz * waveMult[3]));
  var gg = wave((t1 + rr) + (cx * waveMult[2] + cy * waveMult[3] + cz * waveMult[5]));
  var bb = wave((rr + gg) - (cx * waveMult[4] + cy * waveMult[5] + cz * waveMult[1]));
