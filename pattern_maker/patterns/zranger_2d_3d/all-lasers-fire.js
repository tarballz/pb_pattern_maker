...
export function render3D(index, x, y, z) {
  // 3D: the lasers fire from the -y direction. Distance uses hypot3 so
  // the beams radiate out into the volume instead of a flat plane, and
  // the three color layers are offset slightly along z so you see a
  // chromatic separation with depth.
  var r, g, b;
  var px, py, pz, tmp;
  var len;
  ...
  len = hypot3(px, py, pz);
  var zv = time2;
  tmp = len * wr * (tan(len * zap - zv));
  r = 0.075 / hypot3(mod(px / tmp, 1) - 0.55, mod(py / tmp, 1) - 0.55, mod(pz / tmp, 1) - 0.55) / len;
  r = r * r * r;

  // green
  px -= colorSpacing; py -= colorSpacing; pz -= colorSpacing;
  len -= dHyp;
  zv += dT;
  tmp = len * wg * (tan(len * zap - zv));
  g = 0.075 / hypot3(mod(px / tmp, 1) - 0.55, mod(py / tmp, 1) - 0.55, mod(pz / tmp, 1) - 0.55) / len;
  g = g * g * g;
  ...
  len -= dHyp;
  zv += dT;
  tmp = len * wb * (tan(len * zap - zv));
  b = 0.075 / hypot3(mod(px / tmp, 1) - 0.55, mod(py / tmp, 1) - 0.55, mod(pz / tmp, 1) - 0.55) / len;
  b = b * b * b;
