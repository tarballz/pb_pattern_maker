/* Rose Curve 2D/3D
 ...
 N and D sliders control angular frequency. The curve lives in the xy plane;
 in 3D the curve is drawn on a thin slab through the center of the volume
 (along z), with a smooth fade at the edges.
...
// thickness of the z-slab the rose is drawn on (half-thickness)
var zSlab = 0.12;
...
// slab thickness slider: how fat the rose appears along z
...
function coolFrameBuffer(delta) {
  ...
  for (var y = 0; y < height; y++) {
    for (var x = 0; x < width; x++) {
      var nn = frameBuffer[x][y] % 1;
      if (!nn) continue;
      frameBuffer[x][y] = floor(frameBuffer[x][y]) + max(0, nn - coolingRate);
    }
  }
}
...
export function render3D(index, x, y, z) {
  ...
  h = floor(v) / 1000;
  v = v % 1;
  // smooth slab along z: full brightness inside the slab, fades to 0 outside.
  zFade = 1 - smoothstep(zSlab, zSlab + 0.1, abs(z - 0.5));
  hsv(h, 1, v * zFade);
}
