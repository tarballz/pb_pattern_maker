...
// on displays with more pixels. 4096 finishes a full search in a single
...
export var mapIndex, tgtIndex, tgtX, tgtY, tgtZ //which pixel is being mapped
...
// Palette crossfade state — blend between palettes smoothly over
// paletteTransitionSeconds instead of snapping to black + new palette.
var PALETTE_SIZE = 16
var blendedPalette = array(4 * PALETTE_SIZE)
var pixel1 = array(3)
var pixel2 = array(3)
var paletteTransitionSeconds = 3
var inPaletteTransition = 0
var paletteBlend = 0
var nextPalette = 0

// User-space copy of Pixelblaze's paint() that writes the interpolated
// rgb into rgbArray instead of the active LED. Used to sample both
// palettes during a crossfade.
...
// Write a blend of pal1 and pal2 into blendedPalette in-place. Because
// setPalette holds a reference, the live palette updates with each call.
function buildBlendedPalette(pal1, pal2, blend) {
  ...
  for (var i = 0; i < PALETTE_SIZE; i++) {
    ...
    blendedPalette[entry++] = v
    blendedPalette[entry++] = mix(pixel1[0], pixel2[0], blend)
    blendedPalette[entry++] = mix(pixel1[1], pixel2[1], blend)
    blendedPalette[entry++] = mix(pixel1[2], pixel2[2], blend)
  }
}
...
function loadTargetCoords(index, x, y, z) {
  if (index == tgtIndex) {
    tgtX = x
    tgtY = y
    tgtZ = z
...
export function render3D(index, x, y, z) {
  if (needsMapping) renderMapNeighborhood(index, x, y, z)
...
export function beforeRender(deltaMs) {
  var k
  if (needsMapping) {
    //save previous frame's neighbors into the permanent map
    if (mapIndex != -1) {
      for (var i = 0; i < numNeighbors; i++) {
    ...
    if (pixelsDrawn < pixelCount) {
      // Commit several pixels per frame for a smoother, faster reveal.
      for (var d = 0; d < drawsPerFrame; d++) {
        if (pixelsDrawn >= pixelCount) break
        k = getAvailablePoint()
        ...
        if (k >= 0) {
          ...
          //special case: search completed with no match — seed a new island
          triggerSeedPixel()
        }
        restartSearch()
      }
    } else if (inPaletteTransition) {
      // Crossfade the active palette. Pixel colors are stored as 0..1
      // palette positions, so paint() re-samples them each frame and the
      // egg morphs into the new palette without ever going dark.
      paletteBlend += deltaMs / 1000 / paletteTransitionSeconds
      if (paletteBlend >= 1) {
        currentPalette = nextPalette
        setPalette(allPalettes[currentPalette])
        inPaletteTransition = 0
        paletteBlend = 0
        // Now erase and reseed so a fresh drawing grows in the new palette.
        pixels.mutate(() => -1)
        pixelsDrawn = 0
        for (var si = 0; si < initialSeeds; si++) triggerSeedPixel()
        restartSearch()
        redrawTimer = 0
      } else {
        buildBlendedPalette(allPalettes[currentPalette], allPalettes[nextPalette], paletteBlend)
      }
    } else if (autoRedraw) {
      redrawTimer += deltaMs / 1000
      if (redrawTimer >= redrawSeconds) {
        // Begin a crossfade to a random next palette.
        nextPalette = floor(random(allPalettes.length))
        inPaletteTransition = 1
        paletteBlend = 0
        buildBlendedPalette(allPalettes[currentPalette], allPalettes[nextPalette], 0)
...
function renderMapNeighborhood(index, x, y, z) {
  var tgtDist = hypot3(tgtX - x, tgtY - y, tgtZ - z)

  // Calm progress fill during the ~45s neighborhood-mapping phase: pixels
  // below the progress line glow dim purple, above stay black. No more
  // flashy candidate/reject visualization — just a silent progress bar.
  var progress = (mapIndex + 1) / pixelCount
  hsv(0.75, 0.6, 0.05 * (y < progress))
