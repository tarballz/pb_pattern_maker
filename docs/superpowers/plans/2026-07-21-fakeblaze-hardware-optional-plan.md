...
- `storage.py` — **DONE** (`pattern_maker/fakeblaze/storage.py` + `pattern_maker/test_storage.py`, 17 tests, all passing, one round-tripped through a real `PBP.fromComponents`/`toFile()` blob as the oracle). Device-filesystem-mirror data dir (default `pattern_maker/.fakeblaze/`, **gitignored**): `p/<17-char-id>` raw PBP, `p/<id>.c` saved controls, `config.json`, optional seeded `index.html.gz` (+ `ver.txt` sidecar, outside the virtual FS address space). Path sanitization (`PathSecurityError` on traversal / missing leading `/`). Uses `pixelblaze.PBP` (already in the `pixelblaze-client` dependency) for PBP build/parse instead of reimplementing it — no pattern ID generation needed either, since `pixelblaze-client` always generates IDs client-side (`makeId()`) before sending. This makes HTTP `/list`+`GET` — and therefore `pb backup` — a passthrough. **Verified against the real seed data already captured**: `Storage('.fakeblaze').seeded_version()` returns `"3.51"` and `seeded_webui()` returns the real 320790-byte `/index.html.gz` fetched from hardware on 2026-07-21 — `--seed-fakeblaze` groundwork is functionally proven, not just planned. Next up per the implementation order: `render_worker.mjs` + `renderworker.py`.
